// PetTable.jsx
// Admin table view for managing pets with edit and delete actions

import React from 'react';
import StatusBadge from './StatusBadge.jsx';

export default function PetTable({ pets, locations, onEdit, onDelete }) {
    // Finds location name by ID for display in table
    const getLocationName = (locationId) => {
        const loc = locations.find(l => l.location_id === locationId);
        return loc ? loc.name : 'Unknown';
    };

    // Shows empty state when no pets exist
    if (pets.length === 0) {
        return (
    ...
    // Renders table with pet data and action buttons
    return (
